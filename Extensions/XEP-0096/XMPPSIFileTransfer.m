...
@interface XMPPSIFileTransfer()
{
    ...
    NSDate *_lastTimeExchangeData;
}
@property (nonatomic, assign) NSUInteger recvFileSize;
@property (nonatomic, retain) NSString* recvFileName;
@property (nonatomic, assign) NSUInteger sendFileSize;
@property (nonatomic, retain) NSString* sendFileName;
@property (nonatomic, strong) NSData *fileToSend;
@property (nonatomic, strong) XMPPJID *fileRecipient;
@end
...
- (BOOL)sendNegotiationResponse:(XMPPIQ*)inIq {
    ...
    else if ([@"video/mov" isEqualToString:[[file attributeForName:@"mime-type"] stringValue]] == NSOrderedSame)
    {
        mimeType = kXMPPSIFileTransferMimeTypeMOV;
    }
    ...
    for (NSXMLElement *option in options) {
        ...
        if ([@"http://jabber.org/protocol/bytestreams" isEqualToString:value]) {
            ...
            NSXMLElement *rvalue2 = [NSXMLElement elementWithName:@"value" stringValue:@"http://jabber.org/protocol/ibb"];
            [rfield addChild:rvalue2];
...
- (BOOL)handleServiceDiscoveryRequest:(XMPPIQ*)inIq {
    [TURNSocket setProxyCandidates:[[NSArray alloc] initWithObjects:xmppStream.myJID.domain, nil]];

    turnSocket = [[TURNSocket alloc] initWithStream:xmppStream toJID:self.fileRecipient];
//
...
- (void)initiateFileTransferTo:(XMPPJID*)to withData:(NSData*)data andFileName:(NSString*)fileName {
    ...
    fileToSend = data;
    self.fileRecipient = to;
    self.sendFileName = fileName;
    self.sendFileSize = [data length];
    NSString *uuid = [xmppStream generateUUID];
    ...
    // call delegate for sending event
    [multicastDelegate fileTransfer:self sendingImage:self.sendFileName progressData:self.fileToSend to:self.fileRecipient];
}
...
{    
    ...
    NSXMLElement *si = [NSXMLElement elementWithName:@"si" xmlns:@"http://jabber.org/protocol/si"];
    [si addAttributeWithName:@"id" stringValue:sid];
    [si addAttributeWithName:@"mime-type" stringValue:type];
    [si addAttributeWithName:@"profile" stringValue:@"http://jabber.org/protocol/si/profile/file-transfer"];
    [iq addChild:si];
    
    if ([type isEqualToString:@"video/mov"])
    {
        mimeType = kXMPPSIFileTransferMimeTypeMOV;
            ...
            if ([@"http://jabber.org/protocol/si" isEqualToString:[si xmlns]]) {
                ...
                if ([@"http://jabber.org/protocol/si/profile/file-transfer" isEqualToString:[file xmlns]]) {
                    ...
                    if ([@"http://jabber.org/protocol/feature-neg" isEqualToString:xmlns]) {
                        self.recvFileName = [file attributeStringValueForName:@"name"];
                        self.recvFileSize = [file attributeIntegerValueForName:@"size"];
                        // call delegate for receiving event
                        [multicastDelegate fileTransfer:self receivingImage:self.recvFileName progress:0.0f from:[inIq from]];
...
- (void)turnSocket:(TURNSocket *)sender didSucceed:(GCDAsyncSocket *)socket {
    ...
    if (state == kXMPPSIFileTransferStateSending) {
        [socket writeData:fileToSend withTimeout:30 tag:0];
    } else if (state == kXMPPSIFileTransferStateReceiving) {
        [socket readDataToLength:self.recvFileSize withTimeout:30 tag:0];
    }
    _lastTimeExchangeData = [NSDate date];
}

- (void)turnSocketDidFail:(TURNSocket *)sender {
	NSLog(@"SOCKS5 Connection failed!");
    if (state == kXMPPSIFileTransferStateSending) {
        [multicastDelegate fileTransfer:self didFailSendImage:self.sendFileName to:self.fileRecipient];
    } else if(state == kXMPPSIFileTransferStateReceiving)
    {
        [multicastDelegate fileTransfer:self didFailReceiveImage:self.recvFileName from:senderJID];
    }
    turnSocket = nil;
    state = kXMPPSIFileTransferStateNone;
}
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    ...
    // notify delegate
    [multicastDelegate fileTransfer:self receivedImage:self.recvFileName data:data from:senderJID];
}

- (void)socket:(GCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag
{
    NSLog(@"READING DATA %d", partialLength);
    //[multicastDelegate fileTransfer:self receivingImage:self.recvFileName progress:(partialLength/self.recvFileSize) from:senderJID];
    _lastTimeExchangeData = [NSDate date];
}

-(void)socket:(GCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag
{
    NSLog(@"SENDING DATA %d", partialLength);
    _lastTimeExchangeData = [NSDate date];
}
...
- (void)socket:(GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag {
    ...
    mimeType = kXMPPSIFileTransferMimeTypeNone;
    // notify delegate
    [multicastDelegate fileTransfer:self didSendImage:self.sendFileName data:self.fileToSend to:self.fileRecipient];
}
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err {
    NSLog(@"SOCKS5 socket disconnected %@", [err localizedDescription]);
    // check error
    if (err) {
        if (state == kXMPPSIFileTransferStateSending) {
            [multicastDelegate fileTransfer:self didFailSendImage:self.sendFileName to:self.fileRecipient];
...
{
    NSInteger diff = [[NSDate date] timeIntervalSinceDate:_lastTimeExchangeData];
    if (diff <= 5) {
        // extend timeout
        return 10;
    }
    
    return 0;// not extend timeout
}
